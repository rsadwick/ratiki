            ...
            foreach (Enemy enemy in enemies)
            {
                ...
                if (enemy.IsAlive && enemy.BoundingRectangle.Intersects(Player.BoundingRectangle))
                {
                    ...
                    else
                    {
						...
						//Sticky grab:
						/*Vector2 cOfMass = (player.Position + enemy.Position) / 2;

						Vector2 normal1 = new Vector2(enemy.BoundingRectangle.Width * player.BoundingRectangle.Width);
						normal1.Normalize();
						Vector2 normal2 = new Vector2(player.BoundingRectangle.Width  - enemy.BoundingRectangle.Width ) ;
						normal2.Normalize();

						player.Position -= cOfMass;
						player.Position = Vector2.Reflect(player.Position, normal1);
						player.Position += cOfMass;

						/*enemy.Position -= cOfMass;
						enemy.Position = Vector2.Reflect(enemy.Position, normal2);
						enemy.Position += cOfMass;*/
