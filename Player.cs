    ...
    class Player
    {
        ...
        { 
            ...
            foreach (var movableEnemy in level.enemies)
            {
				...
				else if(BoundingRectangle.X == movableEnemy.BoundingRectangle.X )
                {

					velocity.X = JumpLaunchVelocity * (5.0f - (float)Math.Pow(jumpTime / MaxJumpTime, JumpControlPower * 2));
					Position -= (movableEnemy.Velocity + movableEnemy.Position)  * 2 / 26;

				}

				/*else if(BoundingRectangle.Left == movableEnemy.BoundingRectangle.Right || 
				        BoundingRectangle.Left == movableEnemy.BoundingRectangle.Left)
				{
